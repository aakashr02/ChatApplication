...
void * handle_client(void *arg) {
    ...
    while ((bytes_read = read(client.socket, buffer, sizeof(buffer) - 1)) > 0) {
        ...
        else 
        {
        	flag=1;
        	char server_name[50];
        	sprintf(server_name, "Server %c waiting to chat with Client %d", serverName, ntohs(client.addr.sin_port)); 
	...
	read(client.socket, buffer, 1);  
	//printf("SERVER RATING : %d\n", (int)buffer[0]-'0');
    ...
    return NULL; //(int)buffer[0]-(int)'0';
}
